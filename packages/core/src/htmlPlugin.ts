...
export function createPlugin(userOptions: UserOptions = {}): PluginOption {
  ...
  return {
    ...
    async closeBundle() {
      ...
      let resetDir = false;
      // Moving all the html from `template` dir to `viteConfig.build.outDir`
      await Promise.all(
        htmlFiles.map((file, index) => {
          // If filename exist use the dir and file name set by filename
          const dest = isMpa(viteConfig)
            ? pages[index].filename
              ? path.resolve(cwd, path.dirname(pages[index].filename))
              : cwd
            : filename
            ? path.resolve(cwd, path.dirname(filename))
            : cwd;
          ...
          const destFile = isMpa(viteConfig)
            ? pages[index].filename
              ? pages[index].filename
              : file
            : filename
          ...
          fs.move(file, path.resolve(dest, path.basename(destFile)), {
            overwrite: true
          });
        })
      );

      // Removing the dir and files inside the `template` dir
...
/**
 * Getting all the input html files
 */
export function createInput(
  ...
  if (isMpa(viteConfig) || pages?.length) {
    ...
    templates.forEach((temp) => {
      ...
      // dirname could be the same for all the templates
      // Therefore use `dirName + filename` as the key index
      // to avoid overwritten each other.
